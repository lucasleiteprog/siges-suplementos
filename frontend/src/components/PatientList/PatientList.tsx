                ...
                filteredPatients.map((patient) => (
                  <tr key={patient.id} onClick={() => navigate(`/pacientes/editar/${patient.id}`)} className="hover:bg-blue-50 cursor-pointer transition-colors group">
                    ...
                    <td className="px-3 py-3 whitespace-nowrap">
                      ...
                      <div className="text-xs text-gray-500">SUS: {patient.cartao_sus || '-'}</div>
                    </td>
                    <td className="px-3 py-3 whitespace-nowrap">
                      ...
                      <div className="text-xs text-gray-500 ml-4">{patient.bairro || 'Bairro não inf.'}</div>
                    </td>
                    <td className="px-3 py-3">
                      <div className="flex flex-wrap gap-1">
                        {patient.formulas && patient.formulas.length > 0 ? (
                          <>
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800 border border-purple-200 max-w-[150px] truncate" title={patient.formulas.map((f:any)=>f.nome).join(', ')}>
                              {patient.formulas[0].nome}
                            </span>
                            {patient.formulas.length > 1 && (
                              <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-gray-100 text-gray-600 border border-gray-200 cursor-help" title={patient.formulas.map((f:any)=>f.nome).join(', ')}>
                                +{patient.formulas.length - 1}
                              </span>
                            )}
                          </>
                    ...
                    <td className="px-3 py-3 whitespace-nowrap">
                      {patient.folder ? (
                        <div className="flex items-center text-xs font-medium text-blue-700 bg-blue-50 px-2 py-1 rounded-md w-fit border border-blue-100">
